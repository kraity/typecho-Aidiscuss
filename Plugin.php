 ...
 * @author 权那他
 * @version 1.0
 * @link https://github.com/kraity/typecho-Aidiscuss
 */
class Aidiscuss_Plugin implements Typecho_Plugin_Interface
{
    ...
    {
        Typecho_Plugin::factory('Widget_Feedback')->comment = array("Aidiscuss_Plugin", 'startDiscuss');
        Typecho_Plugin::factory('Widget_Feedback')->trackback = array("Aidiscuss_Plugin", 'startDiscuss');
        Typecho_Plugin::factory('Widget_XmlRpc')->pingback = array("Aidiscuss_Plugin", 'startDiscuss');
    ...
    {
        if (!$isInit) {
            $before = Typecho_Widget::widget('Widget_Options')->plugin("Aidiscuss");
            $config['authObj'] = $before->authObj;
        }
        Helper::configPlugin("Aidiscuss", $config);
    }
     ...
     */
    public static function startDiscuss($comment, $post)
    {
        $option = Helper::options()->plugin('Aidiscuss');
        ...
        /** 垃圾过滤 */
        $comment = Aidiscuss_Plugin::filter($option, $comment);
        ...
        $result = Aidiscuss_Plugin::analyzeResult($conclusion, $comment);
        $comment = $result["comment"];
        if ($result["state"]) {
            return $comment;
        } else {
            throw new Typecho_Exception($result["error"]);
        }
    }
    ...
    private static function filter($filter_set, $comment)
    {
        ...
        if ($opt == "none" && $filter_set->opt_au_length != "none") {
            if (Aidiscuss_Plugin::strLength($comment['author']) < $filter_set->au_length_min) {
                $error = "昵称请不得少于" . $filter_set->au_length_min . "个字符";
                $opt = $able = $filter_set->opt_au_length;
            } else if (Aidiscuss_Plugin::strLength($comment['author']) > $filter_set->au_length_max) {
        ...
        if ($opt == "none" && $filter_set->opt_length != "none") {
            if (Aidiscuss_Plugin::strLength($comment['text']) < $filter_set->length_min) {
                $error = "评论内容请不得少于" . $filter_set->length_min . "个字符";
                $opt = $filter_set->opt_length;
            } else if (Aidiscuss_Plugin::strLength($comment['text']) > $filter_set->length_max) {
     ...
     * @return array
     */
    private static function analyzeResult($result, $comment)
    {
        $state = true;
        $error = "";
        /** "conclusionType":0 失败响应，1 成功，2 非法*/
